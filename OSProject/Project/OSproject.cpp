...
void display(int arr[noOfStudents], sf::RenderWindow &window, double total_t) {
	...
	if (!backgroundTexture.loadFromFile("1363709.png")) {
		std::cerr << "Failed to load background image" << std::endl;
	}
	sf::Sprite backgroundSprite(backgroundTexture);

	sf::Font font;
	if (!font.loadFromFile("g-ari-bd.ttf")) {
...
int main() {
	...
	while (window.isOpen()) {
		...
		if (currentState == State::MainMenu) {
			...
			sf::Text text;
			text.setFont(font);
			text.setString("Project Creators: Muhammad Huzaifa : Usman Tanveer : Hamza Haroon");
...
// Adjust the position of the text to top-center align it
                        text.setPosition(centerX - textBounds.width / 2, 10);
